...
//! [`crate::config::outputs_mut::delete`] with a scope hint derived from which
...
impl CreateEditor {
    fn from_existing(
        ...
        let total_entries = def.selectors.len() + def.aliases.len();
        if total_entries != 1 {
            return Err(format!(
                "edit not yet supported for multi-entry outputs ({} entries)",
                total_entries
            ));
        }
        ...
        if let Some(sel) = def.selectors.first() {
            let path = &sel.0;
            if path.starts_with("tag:") || path.contains('+') || path.contains(',') {
                return Err("edit not yet supported for tag selectors — use the DSL editor".into());
            } else if path.ends_with("/*") {
                editor.kind = EntryKind::Glob;
                editor.path = path.trim_end_matches("/*").to_string();
            } else {
                editor.kind = EntryKind::Single;
                editor.path = path.clone();
            }
        } else if let Some((key, path)) = def.aliases.iter().next() {
            if path.starts_with("tag:") || path.contains('+') || path.contains(',') {
                return Err("edit not yet supported for tag selectors — use the DSL editor".into());
            }
    ...
    fn entries(&self) -> OutputDef {
        let path = self.path.trim().trim_end_matches("/*").to_string();
        match self.kind {
            EntryKind::Single => OutputDef {
                selectors: vec![SelectorEntry(self.path.trim().to_string())],
                aliases: AliasMap::new(),
            },
            EntryKind::Glob => OutputDef {
                selectors: vec![SelectorEntry(format!("{path}/*"))],
                aliases: AliasMap::new(),
            },
            EntryKind::Alias => {
...
pub struct OutputsView {
    ...
    entries: BTreeMap<(u8, String), OutputDef>,
    ...
    available_secrets: Vec<SecretCandidate>,
...
impl OutputsView {
    ...
    fn reload(&mut self) {
        ...
        let (project_rows, project_path) = match outputs_mut::read_outputs(ScopeHint::Auto, &cwd) {
            Ok((resolved, envs)) if resolved.scope == Scope::Project => {
                let path = resolved.config_path.clone();
                let mut rows: Vec<(String, OutputDef)> = envs.into_iter().collect();
        ...
        if let Ok((_resolved, envs)) = outputs_mut::read_outputs(ScopeHint::Global, &cwd) {
            self.rows.push(Row::Header {
                scope: Scope::Global,
            });
            let mut rows: Vec<(String, OutputDef)> = envs.into_iter().collect();
        ...
        // an empty branch.
        self.available_secrets = crate::cli::search::search_core(&self.ctx, "", &[])
            .unwrap_or_default()
            .into_iter()
            .map(|r| SecretCandidate {
                path: r.path,
    ...
    pub fn on_key(&mut self, key: KeyEvent, keymap: &KeyMap) -> OutputsAction {
    ...
    /// env.
    fn open_dsl_editor(&mut self) -> OutputsAction {
        let initial_yaml = match self.selected_label_scope() {
            Some((label, scope)) => {
                let entries = self
                    ...
                    .cloned()
                    .unwrap_or_default();
                let mut single: BTreeMap<String, OutputDef> = BTreeMap::new();
                single.insert(label.to_string(), entries.clone());
    ...
    fn handle_dsl_key(&mut self, key: KeyEvent, keymap: &KeyMap) -> OutputsAction {
        // Ctrl-C still quits.
        if keymap.quit.matches(&key) && key.code != KeyCode::Esc {
            return OutputsAction::Quit;
        }
        ...
        match editor.on_key(key, &corpus) {
            DslEditorOutcome::Pending => OutputsAction::None,
    ...
    fn perform_dsl_save(&mut self) -> OutputsAction {
        let Some(editor) = self.dsl.as_ref() else {
            return OutputsAction::None;
        };
        let envs = match editor.parse_envs() {
            Ok(envs) => envs,
            Err(e) => return OutputsAction::CreateFailed(format!("parse failed: {e}")),
        };
        if envs.is_empty() {
            return OutputsAction::CreateFailed("editor is empty — nothing to save".into());
        }
        ...
        for (label, entries) in &envs {
            ...
            for (concrete, _value) in env_dsl::expand_brace_label(label) {
                if let Err(e) = validate_env_label(&concrete) {
                    return OutputsAction::CreateFailed(format!("invalid label '{concrete}': {e}"));
                }
            }
            match outputs_mut::upsert_output_entry(label, entries.clone(), ScopeHint::Auto, &cwd) {
                Ok(resolved) => {
        ...
        match (last_label, last_resolved_scope) {
            (Some(label), Some(scope)) => OutputsAction::Created { label, scope },
            _ => OutputsAction::None,
        }
    }

    fn open_edit_selected(&mut self) -> OutputsAction {
        ...
        match CreateEditor::from_existing(&label, scope, &entries) {
            ...
            Err(msg) => OutputsAction::CreateFailed(msg),
        }
    }

    fn handle_create_key(&mut self, key: KeyEvent, keymap: &KeyMap) -> OutputsAction {
        if keymap.quit.matches(&key) && key.code != KeyCode::Esc {
            return OutputsAction::Quit;
        }
    ...
    /// Execute a confirmed delete through [`outputs_mut::delete`] and reload.
    fn perform_delete(&mut self, label: String, scope: Scope) -> OutputsAction {
        let cwd = std::env::current_dir().unwrap_or_else(|_| PathBuf::from("."));
        let hint = match scope {
            Scope::Project => ScopeHint::Project,
            Scope::Global => ScopeHint::Global,
        };
        match outputs_mut::delete_output(&label, hint, &cwd) {
            Ok(_) => {
                self.reload();
                OutputsAction::Deleted { label, scope }
            }
            Err(e) => OutputsAction::DeleteFailed(format!("delete failed: {e}")),
        }
    }

    fn perform_create(&mut self) -> OutputsAction {
        let Some(editor) = self.create.clone() else {
            return OutputsAction::None;
        };
        if let Some(msg) = editor.validation_error() {
            return OutputsAction::CreateFailed(format!("create failed: {msg}"));
        }

        let label = editor.label.trim().to_string();
        let cwd = std::env::current_dir().unwrap_or_else(|_| PathBuf::from("."));
        match outputs_mut::upsert_output_entry(&label, editor.entries(), editor.scope_hint, &cwd) {
            Ok(resolved) => {
                ...
                OutputsAction::Created {
                    label,
                    scope: resolved.scope,
                }
            }
            Err(e) => OutputsAction::CreateFailed(format!("create failed: {e}")),
        }
    }
                ...
                for (i, output) in outputs.iter().enumerate() {
                    ...
                    for entry in &output.entries {
                        lines.push(Line::from(vec![
                            ...
                            Span::styled(" = ", Style::default().fg(theme::muted())),
                            Span::raw(entry.secret_path.clone()),
                        ]));
                    }
                }
            }
...
fn build_corpus(items: &[SecretCandidate]) -> Vec<String> {
...
mod tests {
    ...
    fn renders_project_and_global_sections() {
        ...
        let view = OutputsView::new(&ctx_in(&empty_store));
        ...
        assert_eq!(dev.selectors.len() + dev.aliases.len(), 1);
    }
    ...
    fn d_then_y_invokes_delete_and_reloads() {
        ...
        let mut view = OutputsView::new(&ctx_in(&empty_store));
        ...
        assert!(matches!(act, OutputsAction::None));
    ...
    fn create_single_entry_saves_and_reloads() {
        ...
        assert_eq!(entries.selectors[0].0, "stage/API_KEY");
    }
    ...
    fn edit_existing_env_replaces_entries() {
        ...
        let entries = view
            ...
            .get(&(scope_key(Scope::Project), "dev".to_string()))
            .expect("dev entries present");
        assert_eq!(entries.selectors[0].0, "dev/NEW_API");
    }
    ...
    fn edit_label_change_relocates_row() {
        ...
        let act = view.on_key(ctrl('s'), &km);
        assert!(matches!(act, OutputsAction::Created { .. }));
    ...
    fn edit_multi_entry_env_returns_error() {
        ...
        let act = view.on_key(key(KeyCode::Char('e')), &km);
        match act {
            OutputsAction::CreateFailed(msg) => assert!(msg.contains("multi-entry")),
