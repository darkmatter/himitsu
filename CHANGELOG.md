...
- All secret-path subcommands (`get`, `read`, `set`, `write`, `ls`, `rekey`, `exec`) now emit `himitsu __complete-paths --fuzzy` in generated shell completions; previously `exec` was explicitly excluded from fuzzy matching