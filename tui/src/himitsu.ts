...
const BIN =
  ...
  resolve(dirname(new URL(import.meta.url).pathname), "../../target/release/himitsu")

export interface InitResult {
  user_home: string
  store: string
  pubkey: string
  home_existed: boolean
...
export function getSecret(env: string, key: string, store?: string): string | null {
  return tryRun([...storeArgs(store), "get", env, key])
}

export function setSecret(env: string, key: string, value: string, store?: string): string {
